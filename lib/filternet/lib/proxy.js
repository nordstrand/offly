...
//               request.connection.remoteAddress || request.connection.socket.remoteAddress;
...
//           headers['forwarded-for'] = headers['x-forwarded-for'];
           ...
           var runRequest = function (requestInfo) {
               ...
               requestInfo['rejectUnauthorized'] = false;
                      ...
                      proxyResponse.on('end', function () {
                           ...
                           var setupIntercept = function (error, newBuffer) {
                               ...
                               emitOrRun('interceptResponseContent', function () { writeResponse(newBuffer); },
                                         newBuffer, proxyResponse, isSsl, charset, requestInfo, writeResponse);
                           };
           ...
           emitOrRun('interceptRequest', function () { runRequest(requestInfo); }, requestInfo, response, runRequest);
    ...
    var connect = requireFromRoot('node_modules/connect');
    var morgan = requireFromRoot('node_modules/morgan');
    
    var app = connect();
    
    if( requireFromRoot("./lib/options.js").cli.httplog) {
        app.use(morgan('dev'));
    }
    
    app.use(serverDefinition(false));
    ...
    httpServer.listen(mainPort, hostname, undefined, function() {
        deferred.resolve();
    });
    listenPromises.push(deferred.promise);
    ...
    emitter.isListening = Q.all(listenPromises);
