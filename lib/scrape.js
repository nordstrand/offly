...
exports.start = function(file) {
    startupPhantom()
    .then(function() {
        return offly.start(['dump', '--file', file]);
    })
    .then(function() {
        console.log("Crawling '" + options.cli["crawl_url"] + "'.");
        return getUrls(options.cli["crawl_url"]);
    })
    .then(function(urls) {
        var fixedUrls = fixupUrls(urls);
...
function fixupUrls(urls) {
    
    return _(urls).chain()
        .uniq()
        .filter(httpUriSchemes)
        .filter(onOriginalDomain)
        .value();
}


function onOriginalDomain(url) {
    return (parseURL(url).hostname === parseURL(options.cli["crawl_url"]).hostname);
}
